...
locals {
  ...
  OU_Infra = "New-ADOrganizationalUnit -Name 'Azure-Component' -Path ${var.OUpath_infra}"
  OU_VDA = "New-ADOrganizationalUnit -Name 'Azure-VDA' -Path ${var.OUpath_VDA}"
  shutdown_command     = "shutdown -r -t 10"
  exit_code_hack       = "exit 0"
  powershell_command   = "${local.import_command}; ${local.password_command}; ${local.install_ad_command}; ${local.configure_ad_command};  ${local.OU_Infra}; ${local.OU_VDA}; ${local.shutdown_command}; ${local.exit_code_hack}"
}
